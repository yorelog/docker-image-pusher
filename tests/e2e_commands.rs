...
// Export a docker-save tar via docker CLI, then push it.
// Defaults E2E_EXPORT_IMAGE to the provided alpine reference if not set.
// Optional: E2E_TARGET/E2E_USERNAME/E2E_PASSWORD for auth/override.
#[test]
fn e2e_export_tar_and_push() {
    let image = env::var("E2E_EXPORT_IMAGE")
        .unwrap_or_else(|_| "registry.cn-beijing.aliyuncs.com/yoce/alpine:3.21".to_string());

    // Ensure docker is available.
    let docker_ok = StdCommand::new("docker").arg("version").status();
    if !matches!(docker_ok, Ok(status) if status.success()) {
        eprintln!("skipping: docker CLI unavailable");
        return;
    }

    let tmp = tempfile::tempdir().expect("tempdir");
    let tar_path = tmp.path().join("exported.tar");

    let pull_status = StdCommand::new("docker")
        .arg("pull")
        .arg(&image)
        .status();
    if !matches!(pull_status, Ok(status) if status.success()) {
        eprintln!("skipping: docker pull failed for {}", image);
        return;
    }

    let save_status = StdCommand::new("docker")
        .arg("save")
        .arg(&image)
        .arg("-o")
        .arg(&tar_path)
        .status();
    if !matches!(save_status, Ok(status) if status.success()) {
        eprintln!("skipping: docker save failed for {}", image);
        return;
    }
    ...
    let mut push_cmd = Command::cargo_bin("docker-image-pusher").expect("bin");
    push_cmd.arg("push").arg("--tar").arg(tar_path.to_string_lossy().to_string());
    if let Some(t) = target { push_cmd.arg("--target").arg(t); }
    if let Some(u) = username.as_ref() { push_cmd.arg("--username").arg(u); }
    if let Some(p) = password.as_ref() { push_cmd.arg("--password").arg(p); }
