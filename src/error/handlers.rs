...
impl HttpErrorHandler {
    ...
    pub fn handle_upload_error(status: StatusCode, error_text: &str, context: &str) -> PusherError {
        let error_msg = match status.as_u16() {
            ...
            404 => format!("Repository not found or {} session expired: {}", context, error_text),
    ...
    pub fn handle_registry_error(status: StatusCode, error_text: &str, operation: &str) -> PusherError {
        let error_msg = match status.as_u16() {
            401 => format!("Unauthorized to perform {} operation: {}", operation, error_text),
            403 => format!("Forbidden: insufficient permissions for {}: {}", operation, error_text),
...
impl NetworkErrorHandler {
    ...
    pub fn handle_network_error(error: &reqwest::Error, context: &str) -> PusherError {
        if error.is_timeout() {
            ...
            PusherError::Network(format!("TLS certificate error during {}: {}", context, error))
        } else {
...
impl ValidationErrorHandler {
    ...
    pub fn validate_file_path(file_path: &str) -> Result<()> {
        ...
        if !matches!(extension.to_lowercase().as_str(), "tar" | "tar.gz" | "tgz") {
            return Err(PusherError::Validation(format!(
                "Input file must be a tar archive (.tar, .tar.gz, or .tgz): {}", file_path
            )));
        }
