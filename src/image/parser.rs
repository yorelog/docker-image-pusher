...
//!
//! This module provides [`ImageParser`] for reading Docker image tarballs, extracting layer and config metadata,
//! validating digests, and reporting progress. It is central to the correctness and reliability of the image push process.
...
impl ImageParser {
    ...
    pub async fn parse_tar_file(&mut self, tar_path: &Path) -> Result<ImageInfo> {
        ...
        self.output.info(&format!(
            "Archive size: {}",
            self.output.format_size(file_size)
        ));
        ...
        match parse_result {
            Ok(mut image_info) => {
                ...
                image_info.large_layers_count = image_info
                    .layers
                    .iter()
                ...
                if self.output.verbose {
                    ...
                    // Print in Podman-compatible format when not in verbose mode
                    self.print_podman_format_digests(&image_info);
                }
                Ok(image_info)
            }
            Err(e) => {
    ...
    async fn parse_tar_contents(&mut self, tar_path: &Path) -> Result<ImageInfo> {
        ...
        for entry_result in entries {
                ...
                || path == "manifest.json"
            {
                if size == 0 {
                    self.output
                        .detail(&format!("Entry {}: {} (EMPTY)", entry_count, path));
                } else {
                    self.output.detail(&format!(
                        "Entry {}: {} ({})",
                        entry_count,
            ...
            match self
                .process_tar_entry(&mut entry, &path, size, tar_path)
                .await?
            {
        ...
        self.output
            .info(&format!("Processed {} entries total", entry_count));
        self.output
            .info(&format!("Found {} layer entries", layer_count));

        // Build image info using manifest-provided digests
        let image_info = self
            .build_image_info_with_manifest_digests(manifest_data, config_data, layers)
            .await?;
        Ok(image_info)
    }
    ...
    fn extract_digest_from_layer_path(&self, layer_path: &str) -> Option<String> {
        self.output.detail(&format!(
            "Extracting digest from layer path: {}",
            layer_path
        ));

        if let Some(digest) = DigestUtils::extract_digest_from_layer_path(layer_path) {
            self.output
                .detail(&format!("  ✅ Found digest: {}...", &digest[..16]));
            Some(digest)
        } else {
            self.output
                .detail("  ❌ No valid digest found in layer path");
            None
        }
    }
    ...
    async fn process_layer(
        ...
        // Handle empty layers specially - 使用标准的空文件SHA256
        if size == 0 {
    ...
    async fn build_image_info_with_manifest_digests(
        ...
        let manifest_str = manifest_data.ok_or_else(|| {
            PusherError::ImageParsing("No manifest.json found in archive".to_string())
        })?;
        ...
        if let Some(obj) = image_manifest.as_object() {
            for (key, value) in obj.iter() {
                ...
                self.output
                    .detail(&format!("  - {}: {}", key, value_preview));
            }
        }
        ...
        if let Some(layer_digests) = image_manifest.get("Layers").and_then(|l| l.as_array()) {
            self.output.info(&format!(
                "✅ Found {} layer paths in 'Layers' field",
                layer_digests.len()
            ));
            ...
            for (manifest_index, layer_digest_value) in layer_digests.iter().enumerate() {
                if let Some(layer_file) = layer_digest_value.as_str() {
                    ...
                    if let Some(digest) = extracted_digest {
                        ...
                        // Find exact matching layer by tar path (most reliable)
                        let mut matched_layer = None;
                        for (i, layer) in layers.iter().enumerate() {
                            // Primary match: exact tar path match
                            if layer.tar_path == layer_file {
                                matched_layer = Some(layers.remove(i));
                                self.output
                                    .detail(&format!("  ✅ Exact path match: {}", layer_file));
                                break;
                            }
                        } // If no exact match, try digest-based matching (as fallback)
                        if matched_layer.is_none() {
                            // 先获取所有可能匹配的索引
                            let mut match_index = None;
                            let mut match_tar_path = String::new();

                            for (i, layer) in layers.iter().enumerate() {
                                // Secondary match: digest content match (be more specific)
                                if layer.digest.contains(&digest)
                                    || layer.tar_path.contains(&digest)
                                {
                                    // Additional validation: ensure we're not matching a substring
                                    if digest.len() >= 12 {
                                        // Only match significant digest prefixes
                                        match_index = Some(i);
                                        match_tar_path = layer.tar_path.clone();
                                        break;
                                    }
                                }
                            }
                            ...
                            if let Some(i) = match_index {
                                matched_layer = Some(layers.remove(i));
                                self.output.detail(&format!(
                                    "  ⚠️  Digest-based match: {} -> {}",
                                    match_tar_path, layer_file
                                ));
                            }
                        }

                        if let Some(mut layer) = matched_layer {
                            // 更新为manifest中的正确digest和路径
                            layer.digest = full_digest.clone();
                            layer.tar_path = layer_file.to_string(); // Use the manifest path for consistency
                            self.output.success(&format!(
                                "✅ Matched layer {}: {} -> {}...",
                                manifest_index + 1,
                                layer_file,
                            ...
                            // 创建新层信息，使用manifest中的路径
                            let is_empty = digest
                                == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";

                            // Try to find size from any remaining layer with similar digest
                            let size = if is_empty {
                                ...
                                layers
                                    .iter()
                                    .find(|l| {
                                        l.tar_path.contains(&digest) || l.digest.contains(&digest)
                                    })
                                    .map(|l| l.size)
                                    .unwrap_or(0)
                            };
                            ...
                            self.output.warning(&format!(
                                "⚠️  Creating new layer entry {}: {} ({} bytes)",
                                manifest_index + 1,
                                layer_file,
                                size
                            ));
                        ...
                        return Err(PusherError::ImageParsing(format!(
                            "Could not extract valid SHA256 digest from layer path: {}",
                            layer_file
                        )));
                    }
                }
        ...
        if !found_layer_digests {
            self.output
                .warning("No 'Layers' field found in manifest, using filenames as fallback");
            for (i, layer) in layers.iter_mut().enumerate() {
                if let Some(extracted_digest) = self.extract_digest_from_layer_path(&layer.tar_path)
                {
                    ...
                    self.output.detail(&format!(
                        "Layer {}: Extracted digest from filename: {}...",
                        i + 1,
                        &layer.digest[..23]
                    ));
                } else {
                    self.output.warning(&format!(
                        "Layer {}: Could not extract digest from path: {}",
                        i + 1,
                        layer.tar_path
                    ));
                }
        ...
        for (i, layer) in layers.iter().enumerate() {
            if !layer.digest.starts_with("sha256:") || layer.digest.len() != 71 {
                return Err(PusherError::ImageParsing(format!(
                    "Layer {} has invalid SHA256 digest format: {}",
                    i + 1,
                    layer.digest
                )));
            }
            ...
            if !self.is_valid_sha256_hex(hex_part) {
                return Err(PusherError::ImageParsing(format!(
                    "Layer {} has invalid SHA256 hex digest: {}",
                    i + 1,
                    layer.digest
                )));
            }
        ...
        let (_, config_str) = config_data.ok_or_else(|| {
            PusherError::ImageParsing("No config file found in archive".to_string())
        })?;
        ...
        self.output
            .step(&format!("Config digest: {}...", &config_digest[..23]));

        // 显示所有层的digest总结
        self.output.subsection("Layer Digest Summary");
        for (i, layer) in layers.iter().enumerate() {
            let source = if found_layer_digests {
                "manifest"
            } else {
                "filename"
            };
        ...
        if found_layer_digests {
            self.output
                .success("✅ Using real digests from Docker manifest");
        } else {
            self.output
                .warning("⚠️  Using filename-based digests (may cause upload issues)");
        }

        self.output
            .success("✅ All layer digests validated as proper SHA256 format");
    ...
    pub async fn validate_layer_data(&self, tar_path: &Path, layer: &LayerInfo) -> Result<bool> {
        self.output
            .detail(&format!("Validating layer data: {}", &layer.digest[..23]));

        // Try to extract the layer data and compute its digest
        match TarUtils::extract_layer_data(tar_path, &layer.tar_path) {
            Ok(data) => {
                let computed_digest = DigestUtils::compute_docker_digest(&data);
                let matches = computed_digest == layer.digest;

                if matches {
                    self.output.success(&format!(
                        "✅ Layer data integrity verified: {} bytes",
                        data.len()
                    ));
                } else {
                    self.output
                        .error(&format!("❌ Layer data integrity failed!"));
                    self.output.detail(&format!("  Expected: {}", layer.digest));
                    self.output
                        .detail(&format!("  Computed: {}", computed_digest));
                    self.output
                        .detail(&format!("  Data size: {} bytes", data.len()));
                }
            ...
            Err(e) => {
                self.output.error(&format!(
                    "Failed to extract layer data for validation: {}",
                    e
                ));
                Ok(false)
            }
    ...
    pub fn debug_layer_mapping(&self, manifest_layers: &[String], parsed_layers: &[LayerInfo]) {
        self.output.subsection("Layer-to-Digest Mapping Analysis");

        self.output.detail(&format!(
            "Manifest contains {} layer entries:",
            manifest_layers.len()
        ));
        for (i, layer_path) in manifest_layers.iter().enumerate() {
            let extracted_digest = self.extract_digest_from_layer_path(layer_path);
            self.output.detail(&format!(
                "  {}: {} -> {:?}",
                i + 1,
                layer_path,
        ...
        self.output.detail(&format!(
            "Parsed tar contains {} layer entries:",
            parsed_layers.len()
        ));
        for (i, layer) in parsed_layers.iter().enumerate() {
            self.output.detail(&format!(
                "  {}: {} ({} bytes) -> {}",
                i + 1,
                layer.tar_path,
