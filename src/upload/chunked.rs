...
//!
//! This implementation follows the Docker Registry HTTP API V2 specification
//! with specific optimizations and compatibility fixes for Harbor registry.
//! 
//! Key features:
...
impl ChunkedUploader {
    ...
    pub async fn upload_large_blob(
        ...
        self.output
            .detail(&format!("Starting upload for {} bytes", data.len()));

        // Use direct upload - most reliable method for Docker registries
        self.upload_direct(upload_url, data, expected_digest, token)
            .await
    }
    async fn upload_direct(
        ...
        self.output
            .detail(&format!("Using direct upload for {} bytes", data.len()));

        // Verify data integrity before upload
        self.output
            .detail("Verifying data integrity before upload...");

        // Try to verify integrity, but use more flexible approach if it fails
        match crate::digest::DigestUtils::verify_data_integrity(data, expected_digest) {
            Ok(_) => {
            ...
            Err(e) => {
                ...
                if !is_gzipped {
                    ...
                    if computed == expected_digest {
                        ...
                        // Use the gzipped data for upload
                        return self
                            .try_upload_with_retries(upload_url, &gzipped, expected_digest, token)
                            .await;
                    }
                }

                // Log the error but proceed with upload (might be a false alarm)
        ...
        self.try_upload_with_retries(upload_url, data, expected_digest, token)
            .await
    }

    async fn try_upload_with_retries(
        &self,
        upload_url: &str,
        data: &[u8],
        expected_digest: &str,
        ...
        for attempt in 1..=self.max_retries {
            match self
                .try_upload(upload_url, data, expected_digest, token)
                .await
            {
                Ok(()) => return Ok(()),
                Err(e) => {
                    if attempt < self.max_retries {
                        ...
                        if is_storage_error {
                            ...
                            self.output.warning(&format!(
                                "Storage backend error (attempt {}): {}. Retrying in {}s...",
                                storage_error_count, e, backoff_delay
                            ));
        ...
        let final_error = last_error
            .unwrap_or_else(|| PusherError::Upload("All upload attempts failed".to_string()));

        if storage_error_count > 0 {
            self.output.error(&format!(
                "All {} upload attempts failed due to registry storage issues",
                self.max_retries
            ));
            self.output.info(
                "💡 Consider checking registry storage capacity or contacting the administrator",
            );
        } else {
            self.output
                .error(&format!("All {} upload attempts failed", self.max_retries));
        }
    ...
    async fn try_upload(
        ...
        self.output.detail(&format!(
            "Upload size: {}",
            self.output.format_size(data_size)
        ));
        self.output
            .detail(&format!("Expected digest: {}", normalized_digest));

        // Skip redundant integrity check as we've already verified at a higher level
        self.output.detail(&format!(
            "Uploading with digest: {}",
        ...
        self.output
            .progress(&format!("Uploading {}", self.output.format_size(data_size)));

        let response = request.send().await.map_err(|e| {
            self.output
                .error(&format!("Network error during upload: {}", e));
        ...
        if response.status().is_success() || response.status().as_u16() == 201 {
            ...
            let error_msg = match status.as_u16() {
                400 => {
                    ...
                    } else if error_text.contains("DIGEST_INVALID") {
                        format!(
                            "Digest validation failed - Registry reports uploaded content doesn't match expected digest: {}",
                            error_text
                        )
                ...
                404 => format!(
                    "Repository not found or upload session expired: {}",
                    error_text
                ),
                409 => format!(
                ...
                500 => {
                    if error_text.contains("s3aws") || error_text.contains("DriverName") {
                        ...
                        format!("Registry server error: {}", error_text)
                    }
                }
