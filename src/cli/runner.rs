...
//!
//! This module implements the main workflow for pushing Docker images, including parsing,
//! validation, registry authentication, and orchestrating the upload process. It supports
//! dry-run, verbose/quiet output, and robust error handling for all major steps.
...
impl Runner {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        if self.args.dry_run {
            self.output
                .success("Dry run completed successfully - no data was uploaded");
            return Ok(());
        }
    ...
    async fn create_registry_client(&self) -> Result<RegistryClient> {
        ...
        let auth_config =
            if let (Some(username), Some(password)) = (&self.args.username, &self.args.password) {
    ...
    async fn push_image(
        ...
        for (i, layer) in image_info.layers.iter().enumerate() {
            ...
            if !exists {
                ...
                self.output
                    .success(&format!("Layer {} already exists", i + 1));
            }
        }
        ...
        if existing_blobs.is_empty() {
            self.output
                .info("No existing layers found - full upload required");
        } else {
            self.output.success(&format!(
                "Found {} existing layers ({} total)",
                existing_blobs.len(),
                self.output.format_size(existing_size)
            ));
        }
        ...
        if missing_blobs.is_empty() {
            ...
            if self.args.skip_existing && !existing_blobs.is_empty() {
                self.output.warning(
                    "--skip-existing flag specified, but there are missing layers that need upload",
                );
                self.output
                    .info("Proceeding with upload of missing layers only");
            }

            if self.args.force_upload {
                self.output.warning(
                    "--force-upload specified, uploading all layers regardless of existence",
                );
                missing_blobs = image_info.layers.clone();
                upload_size = image_info.total_size;
                self.output.info(&format!(
                    "Force uploading {} layers ({} total)",
                    missing_blobs.len(),
                    self.output.format_size(upload_size)
                ));
            }
            ...
            if self.args.max_concurrent > 1 && missing_blobs.len() > 1 {
                self.upload_layers_parallel(client, missing_blobs, repository, &token)
                    .await?;
            } else {
                self.upload_layers_sequential(client, missing_blobs, repository, &token)
                    .await?;
            }
        }
        ...
        let config_exists = client
            .check_blob_exists(&image_info.config_digest, repository)
            .await?;

        if !config_exists {
            self.output.step("Uploading image config");
            self.upload_config_blob(client, image_info, repository, &token)
                .await?;
        ...
        self.output
            .step(&format!("Uploading manifest for {}:{}", repository, tag));
        self.upload_manifest_with_token(client, &manifest, repository, tag, &token)
            .await?;
    ...
    async fn upload_layers_parallel(
        ...
        parallel_uploader
            .upload_layers_parallel(
                layers, repository, tar_path, token, // Pass the token
            )
            .await
    }
    ...
    async fn upload_layers_sequential(
        ...
        for (i, layer) in layers.iter().enumerate() {
            ...
            if layer.size == 0 {
                self.upload_empty_layer(client, layer, repository, token)
                    .await?;
            } else if layer.size > self.args.large_layer_threshold {
                self.upload_large_layer_streaming(client, layer, repository, tar_path, token)
                    .await?;
            } else {
                self.upload_regular_layer(client, layer, repository, tar_path, token)
                    .await?;
            }
        }
    ...
    async fn upload_empty_layer(
        ...
        if response.status().is_success() {
            ...
            Err(PusherError::Upload(format!(
                "Empty layer upload failed (status {}): {}",
                status, error_text
            )))
        }
    }
    ...
    async fn upload_config_blob(
        ...
        uploader
            .upload_large_blob(&upload_url, &config_data, &image_info.config_digest, token)
            .await
    }
    ...
    async fn upload_regular_layer(
        ...
        let layer_data = self
            .extract_layer_data_from_tar(tar_path, &layer.tar_path)
            .await?;
        let upload_url = client
            .start_upload_session_with_token(repository, token)
            .await?;
    ...
    /// Extract layer data from tar archive for upload
    ///
    /// 注意：返回的数据始终为 gzip 压缩的 tar 字节流，便于后续 digest 校验和上传。
    async fn extract_layer_data_from_tar(
        &self,
        tar_path: &Path,
        layer_path: &str,
    ) -> Result<Vec<u8>> {
        // 直接调用 TarUtils，保留原始格式
        let data = crate::tar_utils::TarUtils::extract_layer_data(tar_path, layer_path)?;
        self.output.detail(&format!(
        ...
        // 检查并记录数据是否为 gzip 格式
        let is_gzipped = crate::tar_utils::TarUtils::is_gzipped(&data);
        self.output
            .detail(&format!("Extracted data is gzipped: {}", is_gzipped));

        // 计算并记录 SHA256 以便调试
        let computed = crate::digest::DigestUtils::compute_sha256(&data);
        self.output
            .detail(&format!("Extracted data SHA256: {}...", &computed[..16]));
    ...
    async fn extract_config_data_from_tar(
            ...
            .map_err(|e| PusherError::ImageParsing(format!("Failed to read tar entries: {}", e)))?
        {
            ...
            if path == config_filename {
                ...
                entry.read_to_string(&mut config_string).map_err(|e| {
                    PusherError::ImageParsing(format!("Failed to read config data: {}", e))
                })?;
        ...
        Err(PusherError::ImageParsing(format!(
            "Config file {} not found in tar archive",
            config_filename
        )))
    }

    fn create_image_manifest(
        &self,
        image_info: &crate::image::parser::ImageInfo,
    ) -> Result<String> {
        ...
        self.output
            .detail("✅ Created manifest with validated SHA256 digests");

        serde_json::to_string_pretty(&manifest)
            .map_err(|e| PusherError::Parse(format!("Failed to serialize manifest: {}", e)))
    }

    fn calculate_config_size(&self, image_info: &crate::image::parser::ImageInfo) -> Result<u64> {
        // Calculate config size by extracting the config data 
        // or use a reasonable default
        match self.extract_config_data_from_tar(image_info).now_or_never() {
            Some(Ok(data)) => Ok(data.len() as u64),
            _ => Ok(1024) // Default size if we can't get the actual size
        }
    }
}
