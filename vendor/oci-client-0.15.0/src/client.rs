...
impl Client {
    ...
    /// Pushes a blob to the registry by streaming chunks from an [`AsyncRead`] source.
    ///
    /// This helper is useful when the blob cannot be fully loaded into memory.
    /// Callers are responsible for ensuring that authentication has already been
    /// performed via [`Client::auth`].
        ...
        R: AsyncRead + Unpin,
    {
        ...
        let mut location = self.begin_push_chunked_session(image_ref).await?;
        let mut start: u64 = 0;
        ...
        loop {
            ...
            headers.insert(
                "Content-Length",
                format!("{}", bytes_read).parse().unwrap(),
            );
            ...
            let res = RequestBuilderWrapper::from_client(self, |client| client.patch(&location))
                .apply_auth(image_ref, RegistryOperation::Push)
                .await?
                .into_request_builder()
                .headers(headers)
                .body(body)
            ...
            location = self
                .extract_location_header(image_ref, res, &reqwest::StatusCode::ACCEPTED)
                .await?;
        ...
        self.end_push_chunked_session(&location, image_ref, digest)
            .await
    }
