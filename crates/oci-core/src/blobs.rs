    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LayerUploadOptions")
            ...
            .field(
                ...
                &self.large_layer_threshold_bytes,
            )
...
async fn upload_large_layer(
    ...
    if let Some(reporter) = options.progress_reporter.as_ref() {
        ...
        println!(
            ...
            estimated_chunks, chunk_size_mb
        );
    }
    ...
    Some(tokio::spawn(async move {
        ...
        loop {
            ...
            println!(
                ...
                label, percent, sent_mb, layer_size_mb, speed_mbps, eta_display
            );
        }
    }))
}
...
fn bump_chunk_size(current: usize) -> usize {
    if current >= MAX_AUTO_CHUNK_BYTES {
        ...
        current.saturating_mul(2).min(MAX_AUTO_CHUNK_BYTES)
    }
}
