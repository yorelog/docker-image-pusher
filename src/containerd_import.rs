...
pub async fn run_push_containerd(
    ...
    let resolved = store
        .resolve_image(image)
        .map_err(|e| PusherError::push_error(format!("Failed to resolve image in containerd: {e}")))?;
    ...
    for layer in &manifest.layers {
        ...
        let meta = fs::metadata(&path).map_err(|e| {
            PusherError::push_error(format!("Missing layer {} at {}: {e}", digest, path.display()))
        })?;
    ...
    client
        .push_manifest_bytes(&target_ref, &manifest_payload.manifest_media_type, &manifest_payload.manifest_bytes, auth.as_ref())
        .await
...
pub async fn list_images(root: Option<&str>, namespace: &str) -> Result<(), PusherError> {
    ...
    for img in images {
        println!("- {} -> {} (mediaType={}, size={})", img.name, img.target.digest, img.target.media_type, img.target.size);
    }
    Ok(())
}
...
pub async fn export_images(
    ...
    if digest.is_some() && images.len() != 1 {
        return Err(PusherError::push_error("--digest can only be used with a single image"));
    }
...
fn export_by_manifest(store: &ContainerdStore, digest: &str, out: &str) -> Result<(), PusherError> {
    ...
    if !manifest_src.exists() {
        return Err(PusherError::push_error(format!("Manifest blob not found: {}", manifest_src.display())));
    }
    ...
    let mirror_root = out_root
        ...
        .join("blobs");
    std::fs::create_dir_all(&mirror_root).map_err(|e| PusherError::push_error(format!(
...
fn copy_meta_db(store: &ContainerdStore, out_root: &PathBuf) -> Result<(), PusherError> {
    ...
    if let Some(parent) = meta_dst.parent() {
        std::fs::create_dir_all(parent).map_err(|e| PusherError::push_error(format!(
            "Failed to create dir {}: {e}", parent.display()
        )))?;
    }
    if meta_src.exists() {
        std::fs::copy(&meta_src, &meta_dst).map_err(|e| PusherError::push_error(format!(
            "Failed to copy meta.db: {e}")))?;
    }
    Ok(())
}

fn copy_blob_from_store(store: &ContainerdStore, mirror_root: &PathBuf, digest: &str) -> Result<(), PusherError> {
    let digest_ref = containerd_store::DigestRef::parse(digest)
        .map_err(|e| PusherError::push_error(format!("Invalid digest: {e}")))?;
...
fn copy_file(src: &PathBuf, dst: &PathBuf) -> Result<(), PusherError> {
    ...
    std::fs::copy(src, dst).map_err(|e| PusherError::push_error(format!(
        "Failed to copy {} -> {}: {e}", src.display(), dst.display()
    )))?;
    Ok(())
}
...
fn load_image_manifest(store: &ContainerdStore, desc: &containerd_store::Descriptor) -> Result<ManifestPayload, String> {
    let content_root = store.content_root();
    load_manifest_from_desc(&content_root, desc, 0)
}

fn load_manifest_from_desc(content_root: &PathBuf, desc: &containerd_store::Descriptor, depth: usize) -> Result<ManifestPayload, String> {
    if depth > 4 {
    ...
    if !is_manifest_media_type(&desc.media_type) {
        return Err(format!("unsupported manifest mediaType: {}", desc.media_type));
    }
...
fn select_target(source_image: &str, target: Option<String>, registry_override: Option<String>) -> String {
